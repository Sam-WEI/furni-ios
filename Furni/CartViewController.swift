...
public typealias STPTokenSubmissionHandler = (PKPaymentAuthorizationStatus, NSError?) -> Void
...
final class CartViewController: UITableViewController, PKPaymentAuthorizationViewControllerDelegate {
    ...
    @IBAction func beginPayment(_ sender: AnyObject) {
        ...
        paymentRequest.paymentSummaryItems = [
            PKPaymentSummaryItem(label: "Subtotal", amount: NSDecimalNumber(value: cart.subtotalAmount())),
            PKPaymentSummaryItem(label: "Shipping", amount: NSDecimalNumber(value: cart.shippingAmount())),
            PKPaymentSummaryItem(label: "Furni", amount: NSDecimalNumber(value: cart.totalAmount()))
        ]
        ...
        apiClient.createToken(with: payment, completion: { token, error in
            ...
            self.createBackendChargeWithToken(token, completion: { result, error in
                ...
                for item in self.cart.items {
                    Answers.logPurchase(withPrice: NSDecimalNumber(value: item.product.price),
                        currency: "USD",
                        success: true,
    ...
    func createBackendChargeWithToken(_ token: STPToken, completion: @escaping STPTokenSubmissionHandler) {
        ...
        let chargeParams: [String: Any] = ["stripeToken": token.tokenId, "amount": orderPriceCents]

        // Create the POST request to the backend to process the charge.
        
        request(url, method: HTTPMethod.post, parameters: chargeParams).responseJSON() { response in
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cartItem = cart.items[(indexPath as NSIndexPath).row]
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        cart.items.remove(at: (indexPath as NSIndexPath).row)
